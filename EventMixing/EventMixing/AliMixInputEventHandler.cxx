...
Bool_t AliMixInputEventHandler::Notify()
{
    ...
    //
    AliDebug(AliLog::kDebug+5, Form("<-"));
    if(fUseDefautProcess) { AliDebug(AliLog::kDebug, Form("-> SKIPPED")); return AliMultiInputEventHandler::Notify();}
    AliDebug(AliLog::kDebug+5, Form("->"));
    return kTRUE;
}
...
Bool_t AliMixInputEventHandler::Notify(const char *path)
{
    ...
    if(fEventPool && fEventPool->NeedInit())
        fEventPool->Init();
    if(fUseDefautProcess) { AliDebug(AliLog::kDebug, Form("-> SKIPPED")); return AliMultiInputEventHandler::Notify(path);}
    AliDebug(AliLog::kDebug+5, Form("->"));
    return kTRUE;
}
...
Bool_t AliMixInputEventHandler::BeginEvent(Long64_t entry)
{
    ...
    if(fUseDefautProcess) { AliDebug(AliLog::kDebug, Form("-> SKIPPED")); AliMultiInputEventHandler::BeginEvent(entry);/* return GetEntry();*/}
    AliDebug(AliLog::kDebug+5, Form("->"));
    return kTRUE;
}
//_____________________________________________________________________________
...
//     if (fEventPool && fEventPool->NeedInit()) fEventPool->Init();
    if(!fEventPool) { MixStd(); }
    // if buffer size is higher then 1
    else if(fBufferSize > 1) { MixBuffer(); }
    // if mix number is higher then 0 and buffer size is 1
    else if(fMixNumber > 0) { MixEventsMoreTimesWithOneEvent(); }
    else {
