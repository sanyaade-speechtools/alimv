...
)
{
   ...
   AliRsnLoopPair     *loopPhi     = new AliRsnLoopPair(Form("%s_unlike_all" , suffix), pairDef);
   AliRsnLoopPair     *loopPhiMix  = new AliRsnLoopPair(Form("%s_unlike_mix" , suffix), pairDef);
   AliRsnLoopPair     *loopPhiTrue = new AliRsnLoopPair(Form("%s_unlike_phi" , suffix), pairDef);
   ...
   cutRapidity->GetValueObj()->SetSupportObject(pairDef);

   // add cut to pair loops only
   loopPhi    ->GetPairCuts()->AddCut(cutRapidity);
   loopPhi    ->GetPairCuts()->AddCut(cutRapidity);
   loopPhiMix ->GetPairCuts()->AddCut(cutRapidity);
   loopPhiMix ->GetPairCuts()->AddCut(cutRapidity);
   loopPhiTrue->GetPairCuts()->AddCut(cutRapidity);
   loopPhiTrue->GetPairCuts()->AddCut(cutRapidity);

   // simplest scheme: just cut name
   loopPhi    ->GetPairCuts()->SetCutScheme(cutRapidity->GetName());
   loopPhi    ->GetPairCuts()->SetCutScheme(cutRapidity->GetName());
   loopPhiMix ->GetPairCuts()->SetCutScheme(cutRapidity->GetName());
   loopPhiMix ->GetPairCuts()->SetCutScheme(cutRapidity->GetName());
   loopPhiTrue->GetPairCuts()->SetCutScheme(cutRapidity->GetName());
   loopPhiTrue->GetPairCuts()->SetCutScheme(cutRapidity->GetName());
   ...
   AliRsnValue *axisMomTPC = new AliRsnValue("pTPC", AliRsnValue::kTrackPtpc     , 0.0,   5.0, 0.01);
   AliRsnValue *axisSigTPC = new AliRsnValue("sTPC", AliRsnValue::kTrackTPCsignal, 0.0, 500.0, 2.0);
