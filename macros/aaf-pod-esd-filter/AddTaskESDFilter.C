...
//    esdmuonfilter->SetWriteDimuonAOD(writeDimuonAOD);
   ...
   AliAnalysisDataContainer *aodCont = mgr->GetCommonOutputContainer();
  aodCont->SetSpecialOutput();
  mgr->SetSpecialOutputLocation("root://localhost:2094//dev/shm/alice/xrd/data/");
   mgr->ConnectInput  (esdfilter,  0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput (esdfilter,  0, aodCont);
