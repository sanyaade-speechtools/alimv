...
//  AliRsnPairDef         *pairDefmm = new AliRsnPairDef(AliPID::kKaon, '-', AliPID::kKaon, '-', 333, 1.019455);
  AliRsnPairDef         *pairDefpm = new AliRsnPairDef(AliPID::kKaon, '+', AliPID::kPion, '-', 313, 0.892);
  AliRsnPairDef         *pairDefmp = new AliRsnPairDef(AliPID::kKaon, '-', AliPID::kPion, '+', 313, 0.892);
  AliRsnPairDef         *pairDefpp = new AliRsnPairDef(AliPID::kKaon, '+', AliPID::kPion, '+', 313, 0.892);
  AliRsnPairDef         *pairDefmm = new AliRsnPairDef(AliPID::kKaon, '-', AliPID::kPion, '-', 313, 0.892);
...
//   cuts2010->Initialize();
  ...
  AliRsnCutPID *cutPIDK = new AliRsnCutPID("cutPIDKaon", AliPID::kKaon, 0.9);
  cutPIDK->SetPrior(AliPID::kElectron, 0.02);
  cutPIDK->SetPrior(AliPID::kMuon,     0.02);
  cutPIDK->SetPrior(AliPID::kPion,     0.83);
  cutPIDK->SetPrior(AliPID::kKaon,     0.07);
  cutPIDK->SetPrior(AliPID::kProton,   0.06);

  // cut sets
  AliRsnCutSet *cutSetDaughterCommonK = new AliRsnCutSet("commonDaughterCutsK", AliRsnCut::kDaughter);

  // --> add related cuts
  cutSetDaughterCommonK->AddCut(cuts2010);
  cutSetDaughterCommonK->AddCut(cutPIDK);
  // --> define schemes
//   cutSetDaughterCommon->SetCutScheme("cuts2010");
//  cutSetDaughterCommon->SetCutScheme("cutPIDKaon");
   cutSetDaughterCommonK->SetCutScheme("cuts2010&cutPIDKaon");
  if (!cutSetDaughterCommonK->IsValidScheme()) return kFALSE;
  
  // cut managers
  // define a proper name for each mult bin, to avoid omonyme output histos
  pairPMhist->GetCutManager()->SetDaughter1Cuts(cutSetDaughterCommonK);
  pairMPhist->GetCutManager()->SetDaughter1Cuts(cutSetDaughterCommonK);
  pairPPhist->GetCutManager()->SetDaughter1Cuts(cutSetDaughterCommonK);
  pairMMhist->GetCutManager()->SetDaughter1Cuts(cutSetDaughterCommonK);

    // -- tracks --> realistic PID
  AliRsnCutPID *cutPIDPi = new AliRsnCutPID("cutPIDPion", AliPID::kPion, 0.9);
  cutPIDPi->SetPrior(AliPID::kElectron, 0.02);
  cutPIDPi->SetPrior(AliPID::kMuon,     0.02);
  cutPIDPi->SetPrior(AliPID::kPion,     0.83);
  cutPIDPi->SetPrior(AliPID::kKaon,     0.07);
  cutPIDPi->SetPrior(AliPID::kProton,   0.06);

  // cut sets
  AliRsnCutSet *cutSetDaughterCommonPi = new AliRsnCutSet("commonDaughterCutsPi", AliRsnCut::kDaughter);

  // --> add related cuts
  cutSetDaughterCommonPi->AddCut(cuts2010);
  cutSetDaughterCommonPi->AddCut(cutPIDPi);
  // --> define schemes
//   cutSetDaughterCommon->SetCutScheme("cuts2010");
//  cutSetDaughterCommon->SetCutScheme("cutPIDKaon");
   cutSetDaughterCommonPi->SetCutScheme("cuts2010&cutPIDPion");
  if (!cutSetDaughterCommonPi->IsValidScheme()) return kFALSE;
  
  // cut managers
  // define a proper name for each mult bin, to avoid omonyme output histos
  pairPMhist->GetCutManager()->SetDaughter2Cuts(cutSetDaughterCommonPi);
  pairMPhist->GetCutManager()->SetDaughter2Cuts(cutSetDaughterCommonPi);
  pairPPhist->GetCutManager()->SetDaughter2Cuts(cutSetDaughterCommonPi);
  pairMMhist->GetCutManager()->SetDaughter2Cuts(cutSetDaughterCommonPi);
...
//  AliRsnValue *axisPt        = new AliRsnValue("PT", AliRsnValue::kPairPt,             5,  0.0, 5.0);
//  AliRsnValue *axisThetaStar = new AliRsnValue("TS", AliRsnValue::kPairCosThetaStar1, 4, -1.0, 1.0);
...
//  fcnPt   ->AddAxis(axisThetaStar);
  ...
  task->GetAnalysisManager()->Add(pairMPhist);
