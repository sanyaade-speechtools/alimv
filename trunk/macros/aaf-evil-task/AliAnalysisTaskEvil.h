...
public:
    ...
    void NoOutput(Bool_t b=kTRUE,Bool_t btotal=kFALSE) {
        fNoOutput = b;
        fNoOutputTotal = btotal;
    }

    void UseMC(Bool_t useMC=kTRUE) {
        fUseMC = useMC;
    }

    void SetNum1D(Int_t num) {
        if(num > 10000) fNum1D=10000;
        else fNum1D = num;
    }
    void SetNum2D(Int_t num) {
        if(num > 1000) fNum2D=1000;
        else fNum2D = num;
    }
    void SetNum3D(Int_t num) {
        if(num > 100) fNum3D=100;
        else fNum3D = num;
    }
    void SetNumMemoryLeak(Int_t num) {
        fNumMemoryLeak = num;
    }
    void SetHistRange(Int_t bin,Double_t min,Double_t max);
    void SetNumLoopsInOneEvent(const Int_t numLoops) {
        fNumLoopsInOneEvent = numLoops;
    }
    void PrintProcInfo();

    void SetBadHost(TString hostname="",Int_t num=0) {
        fBadHost = hostname;
        fNumMemoryLeakOnSingleNode = num;
    }
private:
