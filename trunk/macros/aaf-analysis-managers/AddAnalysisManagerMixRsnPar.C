...
void AddAnalysisManagerMixRsnPar(TString analysisSource = "proof", TString analysisMode = "test", TString opts = "")
{
   ...
   if (useEventMixingPar) analysisPlugin->SetAdditionalLibs("libXMLParser.so libCORRFW.so EventMixing.par PWG2resonances.par RESONANCESMV.par");
   else analysisPlugin->SetAdditionalLibs("libXMLParser.so libCORRFW.so libEventMixing.so PWG2resonances.par RESONANCESMV.par");
