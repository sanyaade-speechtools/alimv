void AddRsnAnalysisSE_MC(AliLog::EType_t type=AliLog::kInfo)
{
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();

  const Int_t numRsnConfigs=1;
  AliRsnAnalysisSE *task = new AliRsnAnalysisSE("AliRsnAnalysisSE_MC",numRsnConfigs,kTRUE);
...
//   task->SetLogType(type,"AliRsnEvent:AliRsnPair:AliRsnPIDIndex");
  ...
  for (i=0;i<numRsnConfigs;i++)
  {
    ...
    if (analMgr)
      analMgr->AddConfig("RsnConfigDubna_MC.C",Form("PHI%d_MC",i),"RsnConfig_PHI_MC");
  }
  AliAnalysisDataContainer *outputInfo = mgr->CreateContainer("RsnInfo_MC", TList::Class(), AliAnalysisManager::kOutputContainer, "RSNAnalysisInfo_MC.root");
  AliAnalysisDataContainer *outputRsn[numRsnConfigs];
  for (i=0;i<numRsnConfigs;i++)
  {
    outputRsn[i] = mgr->CreateContainer(Form("RsnPhi%d_MC",i), TList::Class(), AliAnalysisManager::kOutputContainer, Form("RSNAnalysis%d.root",i));
